...
def bw(m, m0, gamma):
    ...
    """
    ampl = 1.0 / tf.complex(m0 * m0 - m * m, -m0 * gamma)
...
m = tf.random.uniform((npoints,), minval=0.0, maxval=1500.0, dtype=tf.float64)

# Calculate the vector of 1D Breit-Wigner densities for a generated dataset
#   Use rho0 resonance parameters
y = bw(m, tf.constant(770.0, dtype=tf.float64), tf.constant(150.0, dtype=tf.float64))

# Calculate the maximum of the PDF
ymax = tf.reduce_max(y)

# Generate the uniform random sample from 0 to maximum
r = tf.random.uniform((npoints,), minval=0.0, maxval=ymax, dtype=tf.float64)
