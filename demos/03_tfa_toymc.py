...
phsp = RectangularPhaseSpace(((0.0, 1500.0),))
...
def model(x):
    return bw(x[:, 0], atfi.const(770.0), atfi.const(150.0))


# Run TFA2 toy MC generation
toy_sample = tft.run_toymc(model, phsp, npoints, maximum=1.0e-20, chunk=1000000).numpy()
