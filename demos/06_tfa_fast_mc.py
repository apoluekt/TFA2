...
def random_rotation_and_boost(moms, rnd) : 
  ...
  """
  pt  = -5.*atfi.log(rnd[:,0])   # Random pT, exponential distribution with mean 5 GeV
  eta = rnd[:,1]*3. + 2.         # Uniform distribution in pseudorapidity eta
  phi = rnd[:,2]*2.*atfi.pi()    # Uniform distribution in phi

  theta = 2.*atfi.atan(atfi.exp(-eta)) # Theta angle is a function of eta
  p  = pt/atfi.sin(theta)              # Full momentum
  e  = atfi.sqrt(p**2 + mb**2)         # Energy of the Bs

  px = p*atfi.sin(theta)*atfi.sin(phi) # 3-momentum of initial particle (Bs meson)
  py = p*atfi.sin(theta)*atfi.cos(phi)
  pz = p*atfi.cos(theta)

  boost = atfk.lorentz_vector( atfk.vector(px, py, pz), e)  # Boost vector of the Bs meson

  rot_theta = atfi.acos(rnd[:,3]*2.-1.)  # Random Euler rotation angles for Bs decay
...
def final_state_momenta(data) : 
  ...
  v0jpsi = atfk.vector(zeros, zeros, p0 / ejpsi * ones)  # 3-vector of Jpsi in Bs rest frame
  v0phi = atfk.vector(zeros, zeros, -p0 / ephi * ones)   # 3-vector of phi in Bs rest frame
...
filtered_data = toy_data[
  (atfk.pt(boosted_moms[0])>1.) & 
  (atfk.pt(boosted_moms[1])>1.) & 
  (atfk.pt(boosted_moms[2])>0.5) & 
  (atfk.pt(boosted_moms[3])>0.5) & 
  (atfk.p(boosted_moms[0])>5.) & 
  (atfk.p(boosted_moms[1])>5.) & 
  (atfk.p(boosted_moms[2])>3.) & 
  (atfk.p(boosted_moms[3])>3.) & 
...
fig, ax = plt.subplots(nrows = 1, ncols = 3, figsize = (10, 3) )  # Single subplot on the figure

# Plot 2D histogram from the toy MC sample
tfp.plot_distr2d(filtered_data[:,0], filtered_data[:,1], bins = (50, 50), 
                 ranges = ((-1., 1.), (-1., 1.)), 
                 fig = fig, ax = ax[0], labels = (r"$\cos\theta_{\mu\mu}$", r"$\cos\theta_{KK}$"))

tfp.plot_distr2d(filtered_data[:,0], filtered_data[:,2], bins = (50, 50), 
                 ranges = ((-1., 1.), (0., 2.*math.pi)), 
                 fig = fig, ax = ax[1], labels = (r"$\cos\theta_{\mu\mu}$", r"$\phi$"))

tfp.plot_distr2d(filtered_data[:,1], filtered_data[:,2], bins = (50, 50), 
                 ranges = ((-1., 1.), (0., 2.*math.pi)), 
                 fig = fig, ax = ax[2], labels = (r"$\cos\theta_{KK}$", r"$\phi$"))
